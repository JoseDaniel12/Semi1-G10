...
const getEnviadas = async (req, res) => {
   ...
   `SELECT * FROM usuarios us1
    WHERE id <> ${id}
    AND EXISTS (
        SELECT * FROM amistad am WHERE am.usuario1 = ${id} AND am.usuario2 = us1.id AND am.estado = 0
    )`
...
const getRecibidas = async (req, res) => {
    ...
    WHERE id <> 1
    AND EXISTS (
        SELECT * FROM amistad am WHERE am.usuario2 = ${id} AND am.usuario1 = us1.id AND am.estado = 0
    )`
